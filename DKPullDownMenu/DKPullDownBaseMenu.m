...
- (void)setupBase
{
    ...
    _observer = [[NSNotificationCenter defaultCenter] addObserverForName:DKPullDownMenuTitleDidUpdatedNotification object:nil queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        if (allValues.count == 1 && ![allValues.firstObject isKindOfClass:[NSArray class]]) {
            NSInteger col = [self.controllers indexOfObject:note.object]; // 获取列
            UIButton *btn = self.menuButtons[col]; // 获取对应按钮
            // 设置按钮标题
            [btn setTitle:allValues.firstObject forState:UIControlStateNormal];
        }
        
        // 获取新选中的数据
        DKPullDownResult *result = [DKPullDownResult resultWithTitle:note.userInfo.allKeys.firstObject subTitles:note.userInfo.allValues.firstObject];
        // 丢弃相同项的旧结果
        NSMutableArray *tempRes = [NSMutableArray array];
        if (DKPullDownMenuShareManager.results.count) {
            for (DKPullDownResult *res in DKPullDownMenuShareManager.results) {
                if (![res.title isEqualToString:result.title]) {
                    [tempRes addObject:res];
                }
            }
        }
        // 添加新结果
        [tempRes addObject:result];
        // 更新选择结果
        DKPullDownMenuShareManager.results = [tempRes mutableCopy];
    }];
